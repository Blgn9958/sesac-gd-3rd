...
console.log(fruits);
console.log(fruits[2]); // 2번 인텍스 요소를 뽑아 콘솔에 찍기
...
console.log(b + a); // "23"
console.log(b - a); // -1 -> 문자가 숫자로 변환되어 계산 됨 나머지 * -, /도 동일
...
console.log(introduce.hobby);
console.log(introduce.sesaccrew);
...
let mathscore2 = Number(prompt("수학 점수를 입려해주세요")); // prompt 사용 (사용자에게 값을 입력받아 )
let engscore2 = Number(prompt("영어 점수를 입려해주세요")); // prompt로 입력받은 값은 문자열로 저장되기 때문에 숫자형으로 형변환 함

let avgScore2 = (mathscore2 + engscore2) / 2;
console.log(avgScore2);
alert("당신의 평균 점수 ${avgScore2}점 입니다!");
